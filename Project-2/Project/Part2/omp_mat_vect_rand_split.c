...
int main(int argc, char* argv[]) {
   ...
   double formula = -(((n+1) * (n-(m+m)))/2);
   A = malloc(formula*sizeof(double));
...
void Get_args(int argc, char* argv[], int* thread_count_p, int* m_p, int* n_p)  {
   ...
   *n_p = strtol(argv[3], NULL, 10);
   ...
   if(strcmp("static", argv[4]) == 0)
      omp_set_schedule(omp_sched_static, chunk_size);
   else if(strcmp("dynamic", argv[4]) == 0)
      omp_set_schedule(omp_sched_dynamic, chunk_size);
   else if(strcmp("guided", argv[4]) == 0)
      omp_set_schedule(omp_sched_guided, chunk_size);
   else
      omp_set_schedule(omp_sched_auto, 0);

   if(strcmp(argv[4], "static") != 0 && strcmp(argv[4], "dynamic") && strcmp(argv[4], "guided") && strcmp(argv[4], "auto"))
      Usage(argv[0]);
 ...
 */
void Usage (char* prog_name) {
   fprintf(stderr, "usage: %s <thread_count> <m> <n> <omp_sched_kind> <modifier>\n", prog_name);
   exit(0);
}  /* Usage */
...
void Gen_matrix(double A[], int m, int n) {
   ...
   for (i = 0; i < m; i++)
      for (j = 0; j < (n-i); j++)
            A[i*(n-i)+j] = random()/((double) RAND_MAX);
}  /* Gen_matrix */
...
#  pragma omp parallel for num_threads(thread_count)  \
   ...
   for (i = 0; i < m; i++) {
      y[i] = 0.0;
      //we reduce n by 1 each time to achieve the triagonial traverse
      for (j = 0; j < (n-i); j++) {
         temp = A[i*(n-i)+j]*x[j];
         y[i] += temp;
      }
